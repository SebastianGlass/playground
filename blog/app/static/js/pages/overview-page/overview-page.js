...
export class OverviewPage {
  ...
  render() {
    ...
    const beTemplate = JSON.parse(localStorage.getItem('playground/blog-templates'))['blogentry-preview'];
    const bePlaceholder = document.querySelector('blogentry-preview');
    const blogEntries = this.blogEntries;
    const max = Math.min(blogEntries.length, bePlaceholder.attributes.max.value);
    for (let i = 0; i < max; i++) {
